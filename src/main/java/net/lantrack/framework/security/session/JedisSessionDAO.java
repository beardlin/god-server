...
import net.lantrack.framework.security.web.Servlets;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
    ...
    public void update(Session session) throws UnknownSessionException {
        ...
        if (request != null) {
            ...
            // 如果是视图文件，则不更新SESSION
            if (StrUtil.startsWith(uri, Config.prefix)
                    && StrUtil.endsWith(uri, Config.suffix)) {
                return;
            }
            // 手动控制不更新SESSION
            if (Config.NO.equals(request.getParameter("updateSession"))) {
                return;
            }
        }
        ...
        try {
            ...
            String principalId = pc != null ? pc.getPrimaryPrincipal().toString() : StrUtil.EMPTY;
    ...
    public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession) {
        ...
        try {
            ...
            for (Map.Entry<String, String> e : map.entrySet()) {
                if (StrUtil.isNotBlank(e.getKey()) && StrUtil.isNotBlank(e.getValue())) {

                    String[] ss = StrUtil.split(e.getValue(), "|");
                    if (ss != null && ss.length == 3) {// jedis.exists(sessionKeyPrefix + e.getKey())){
                        ...
                        try {
                            ...
                            if (principal != null) {
                                PrincipalCollection pc = (PrincipalCollection) session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
                                if (principal.toString().equals(pc != null ? pc.getPrimaryPrincipal().toString() : StrUtil.EMPTY)) {
                                    isActiveSession = true;
                                }
                            }
                            // 过滤掉的SESSION
                ...
                // 存储的SESSION无Value
                else if (StrUtil.isNotBlank(e.getKey())) {
                    jedis.hdel(sessionKeyPrefix, e.getKey());
                }
            }
